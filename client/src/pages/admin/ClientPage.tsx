...
import { createClienteService, deleteClienteService, updateClienteService } from '@/services/clients.service'
...
export const ClientPage = () => {
  const { clients, addClientStore, deleteClientStore, updateClientStore } = useClientStore()
  ...
  const renderClients = useMemo(() => {
    return clients.filter((client) => {
      if (values.filter && values.filter.length > 0) {
        return client.names.toLowerCase().includes(values.filter.toLowerCase())
      }
      return client
    })
